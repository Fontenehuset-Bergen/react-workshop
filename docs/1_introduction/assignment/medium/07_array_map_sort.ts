...
console.log(
    `Answer: ${JSON.stringify(trimAll(['  Ada ', ' Linus']))}\t\tExpected: ["Ada","Linus"]`
);
...
console.log(
    `Answer: ${JSON.stringify(
        sortedNamesCaseInsensitive(['b', 'A', 'å'])
    )}\tExpected: ["A","b","å"]`
);
...
console.log(
    `Answer: ${JSON.stringify(
        sortUsersByName(users).map((u) => u.name)
    )}\tExpected: ["ada","Grace","Linus"]`
);
console.log(
    `Answer: ${JSON.stringify(sortUsersByAge(users).map((u) => u.age))}\t\tExpected: [36,45,50]`
);
console.log(
    `Answer: ${JSON.stringify(users.map((u) => u.name))}\t\tExpected: ["Linus","ada","Grace"]`
);
